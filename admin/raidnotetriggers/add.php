 ...
 * -----------------------------------------------------------------------
 * File:        add.php
 * Description	add, update, delete raid note triggers
...
class CTRT_AddRaidNoteTrigger extends CTRT_ManageRaidNoteTriggers
{
    ...
    function CTRT_AddRaidNoteTrigger()
    {
        ...
        $this->data = array(
            ...
            'raid_note_trigger_name'    => $this->_in->get('raid_note_trigger_name'),
            'raid_note_trigger_result'  => $this->_in->get('raid_note_trigger_result')
        );
        ...
        $this->set_vars(array(
            ...
            'url_id'        => ( sizeof($raid_note_trigger_ids) > 0 ) ? $ids : (( $this->_in->exists(URI_ID) ) ? $this->_in->get(URI_ID) : ''),
        ...
        if ( !empty($this->url_id) )
        {
            ...
            $this->data = array(
                ...
                'raid_note_trigger_name'   => $this->_in->get('raid_note_trigger_name', $row[0]['raid_note_trigger_name']),
                'raid_note_trigger_result' => $this->_in->get('raid_note_trigger_result', $row[0]['raid_note_trigger_result']),
            );
        }
    }
    ...
    function error_check()
    {
        if ( ($this->_in->exists('add')) || ($this->_in->exists('update')) ) {
            $this->fv->is_filled('raid_note_trigger_name', $this->getUserLang('fv_required_name'));
            $this->fv->is_filled('raid_note_trigger_result', $this->getUserLang('fv_required_name'));
        }
    ...
    function process_confirm()
    {
        ...
        foreach ($ids as $id) {
            ...
            $success_message .= sprintf($this->getUserLang("ctrt_trigger_success_delete"),$old_data[0]['raid_note_trigger_name'], $old_data[0]['raid_note_trigger_result']) . '<br />';
        }
    ...
    function process_update()
    {
        ...
        $old_data = $this->daoRaidNoteTrigger->getByPrimaryKey($this->url_id);

        if(!$this->daoRaidNoteTrigger->update($this->data)) {
            // Error out if event trigger exists

            $failure_message = sprintf($this->getUserLang("ctrt_trigger_duplicate"),$this->_in->get('raid_note_trigger_name'),$this->_in->get('raid_note_trigger_result'));
            $link_list = array(
					$this->getUserLang('ctrt_adminmenu_list')  => $this->getControllerLink($this->getMyParam(),'list'),
					$this->getUserLang('ctrt_adminmenu_add')  => $this->getControllerLink($this->getMyParam(),'add'),
					$this->getUserLang('ctrt_adminmenu_export')  => $this->getControllerLink($this->getMyParam(),'export'),
            ...
            // Success message
            //
            $success_message = sprintf($this->getUserLang('ctrt_trigger_success_update'), $old_data['raid_note_trigger_name'], $old_data['raid_note_trigger_result']);
            $link_list = array(
					$this->getUserLang('ctrt_adminmenu_list')  => $this->getControllerLink($this->getMyParam(),'list'),
					$this->getUserLang('ctrt_adminmenu_add')  => $this->getControllerLink($this->getMyParam(),'add'),
					$this->getUserLang('ctrt_adminmenu_export')  => $this->getControllerLink($this->getMyParam(),'export'),
    ...
    {
        //Clean up the data
        $name = $this->_in->get('raid_note_trigger_name');
        $result = $this->_in->get('raid_note_trigger_result');
    ...
    function display_form()
    {
        ...
        $this->_tpl->assign_vars(array(
            ...
            // Form values
            'V_ID'          => ( $this->_in->exists('add') ) ? '' : $this->data['raid_note_trigger_id'],
            ...
            'L_RESULT'        => $this->getUserLang('ctrt_raid_note'),
            ...
            'L_HELP_NAME'   => $this->getUserLang('ctrt_raid_note_triggers_help_name'),
            'L_HELP_RESULT' => $this->getUserLang('ctrt_raid_note_triggers_help_result'),
