 ...
 * -----------------------------------------------------------------------
 * File:        dataaccessor.php
 * Description	Base Data Accessor object for CTRT_RaidTracker
...
require_once($eqdkp->root_path.'plugins/'.$pm->get_data('ctrt','path').'/settings.php');
require_once($eqdkp->root_path.'plugins/ctrt/admin/ctrt_base.php');
...
class CTRT_DataAccessor extends CTRT_Base {
    ...
    function getByName ($name) {
        if (isset($name)) {
            return $this->getRecord(mysql_real_escape_string($name));
        } else {
            return 0;
        }
    }
